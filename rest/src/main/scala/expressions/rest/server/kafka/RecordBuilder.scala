...
case class RecordBuilder[K, V](key: K, value: V, producer: Producer, blocking: Blocking, keySerde: Serde[Any, K], valueSerde: Serde[Any, V], partition: Int = -1) {
  ...
  def publishTo(topic: String, timestamp: Long = System.currentTimeMillis()) = {
    ...
    def fail = new Exception(s"Publish timed the fuck out trying to publish to $topic")
    producer.produce(record, keySerde, valueSerde).timeout(60.seconds).either.flatMap {
      case Right(Some(done)) => ZIO.succeed(done)
      case Left(err)         => ZIO.fail(err)
      case Right(None)       => ZIO.fail(fail)
    }
  }
