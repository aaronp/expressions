...
import pipelines.TestWideScheduler

import scala.util.Try

trait RepoTestData extends TestWideScheduler {
  def ints           = DataSource(Observable.fromIterable(0 to 100))
  def strings        = DataSource(Observable.fromIterable(0 to 100).map(_.toString))
