...
class SourceRepositoryTest extends BaseCoreTest with RepoTestData {
  ...
  "Repository.listSources" should {
    "return all results when no content-type is specified" in {
      repo.listSources(ListRepoSourcesRequest(None)).sources should contain only (
    ...
    "return filtered results when a content-type is specified" in {
      SourceRepository("ints" -> ints, "strings" -> strings).listSources(ListRepoSourcesRequest(Some(ContentType.of[String]))).sources should contain only (
  ...
  "Repository.listSinks" should {
    "return a single result by default" in withScheduler { implicit sched =>
      SourceRepository().listSinks(ListSinkRequest()).results should contain only (ListedSink("sink"))
    }
  }
}
