...
object DataSource {
  ...
  def of(contentType: ContentType, obs: Observable[_]): DataSource = new AnonTypeDataSource(contentType, obs)

  def apply[T: TypeTag](observable: Observable[T]): DataSource = {
    apply(ContentType.of[T], observable)
  }

  def apply[T](contentType: ContentType, observable: Observable[T]): DataSource = {
    new SingleTypeDataSource(contentType, observable)
  }
  ...
  class PushSource[A](override val contentType: ContentType, val input: Observer[A], obs: Observable[A]) extends DataSource {
  ...
  private case class AnonTypeDataSource(override val contentType: ContentType, observable: Observable[_]) extends DataSource {
    override def data(ct: ContentType): Option[Observable[_]] = {
      if (ct == contentType) {
        Option(observable)
      } else {
  ...
  private case class SingleTypeDataSource(override val contentType: ContentType, observable: Observable[_]) extends DataSource {
